...
export default function FeedsPage() {
  ...
  return (
    <main className="mx-auto flex max-w-5xl flex-col gap-8 px-4 sm:px-6 py-12">
      ...
      {loading && <p className="text-xs sm:text-sm text-neutral-300 font-mono-tech animate-pulse">Loading FTSO feeds…</p>}
        ...
        {feeds.map((f) => (
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-base sm:text-lg font-bold text-white">{f.symbol}</span>
              <SimulatedBadge simulated={f.simulated} reason={f.simulationReason} />
            </div>
            <span className="font-mono-tech text-2xl sm:text-3xl font-black text-white">
              {f.value != null ? `$${f.value.toLocaleString(undefined, { maximumFractionDigits: 6 })}` : "—"}
            </span>
            <span className="truncate font-mono-tech text-xs text-neutral-400" title={f.feedId}>
              {f.feedId}
            </span>
          </div>
        ))}
